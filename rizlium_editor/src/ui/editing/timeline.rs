...
use egui::{Color32, Stroke, Ui, Id, Sense};
...
pub fn timeline_horizontal(
    ...
    let res = ui.interact(timeline_zone, Id::new("timeline_interact"), Sense::click_and_drag());

    TimeLineResponse {
        seek_to: ((res.is_pointer_button_down_on() && res.drag_delta().x !=0.) || res.clicked() ).then(|| Some(remap_reversed(res.interact_pointer_pos().map(|p| p.x)?))).flatten(),
        range_changed: false,
        scale_changed: false,
    }
}
...
fn line_h(ui: &mut Ui, y: f32, view: egui::Rect, stroke: Stroke) {
    ui.painter_at(view).vline(y, view.x_range(), stroke);
