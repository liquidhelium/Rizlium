...
pub fn process_rune_registrations(
            ...
            world.resource_scope(
                |_world, _registry: Mut<'_, helium_framework::reflect_system::RSystemRegistry>| {
                    let _meta = helium_framework::reflect_system::ReflectSystemMeta {
                        ...
                        system_id:
                            helium_framework::reflect_system::ReflectSystemId::from_system_id(
                                system_id,
                            ),
                        input: "()".to_string(),
                        output: "()".to_string(),
    ...
    for menu in registrar.menus {
        // 注册到helium menu系统
        world.resource_scope(
            |world, mut menu_system: Mut<'_, helium_framework::menu_system::MenuSystem>| {
                menu_system.register(
                    helium_framework::menu_system::MenuItem::new(
                        menu.title.clone(),
                        menu.path.clone(),
                        helium_framework::menu_system::Action::Command(
                            menu.action_id.as_str().into(),
